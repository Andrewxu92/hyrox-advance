...
function generateTrainingPlan(level, weaknesses, strengths, weeks, focusAreas) {
    ...
    for (let weekNum = 1; weekNum <= weeks; weekNum++) {
        const week = generateWeek(weekNum, weeks, level, daysPerWeek, weaknesses, strengths, focusAreas);
        planWeeks.push(week);
    }
...
function generateWeek(weekNum, totalWeeks, level, daysPerWeek, weaknesses, strengths, focusAreas) {
    ...
    for (let dayNum = 1; dayNum <= daysPerWeek; dayNum++) {
        const day = generateDay(dayNum, daysPerWeek, weekNum, phase, level, weaknesses, strengths);
        days.push(day);
    }
...
function generateStrengthDay(dayNum, weekNum, phase, level, weaknesses) {
    ...
    exercises.push({ name: 'Goblet Squats', sets, reps: 12, rest: 90 }, { name: 'Kettlebell Swings', sets, reps: 15, rest: 90 }, { name: 'Farmers Carry', sets: 3, duration: 60, rest: 120 });
...
function generateSkillDay(dayNum, weekNum, phase, level, weaknesses) {
    ...
    exercises.push({ name: 'Burpee Practice', sets: 3, reps: 10, rest: 60, notes: 'Smooth, efficient movement' }, { name: 'Transition Practice', sets: 5, duration: 60, rest: 120, notes: 'Run to station and back' });

 ...
 * 从hyresult.com抓取选手成绩
 */
export async function scrapeHyresult(athleteName, raceLocation) {
    ...
    try {
        ...
        if (raceLocation && athleteLinks.length > 1) {
            const locationMatch = athleteLinks.find(a => a.location.toLowerCase().includes(raceLocation.toLowerCase()));
            if (locationMatch)
                selectedAthlete = locationMatch;
        }
...
function inferGender(name, $) {
    ...
    if (pageText.includes('women') || pageText.includes('female'))
        return 'female';
    if (pageText.includes('men') || pageText.includes('male'))
        return 'male';
...
export async function searchAthleteResults(athleteName) {
    const browser = await puppeteer.launch({
        headless: true,
        args: ['--no-sandbox', '--disable-setuid-sandbox']
    });

...
export async function generateAnalysis(
  ...
  const stationAnalysis: { station: string; displayName: string; time: number; gap: number }[] = [];
...
function analyzePacing(runTimes: number[]) {
  ...
  const runs = runTimes.map((time, index) => {
    ...
    let trend: 'fast' | 'steady' | 'slowing' = 'steady';
...
function calculateConsistencyBonus(runTimes: number[]): number {
...
function generateSummary(level: string, weaknesses: any[], pacingAnalysis: any): string {
...
function generateDefaultRecommendations(weaknesses: any[]): any[] {
